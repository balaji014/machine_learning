...
# Locate project root so model paths work regardless of CWD
PROJECT_ROOT = Path(__file__).resolve().parents[2]
MODEL_PATH = PROJECT_ROOT / "models" / "knn_iris_model.joblib"
...
def predict() -> tuple:
    ...
    try:
        sample = {name: float(payload[name]) for name in EXPECTED_FEATURES}
